...
ADM2_FOI_import_and_format <- function (place, setting, Nb_iter){
  ...
  lambdaNy  <-  readRDS(paste("data/foi/ADM2/Inputs",Nb_iter,"it/Post_FoIPred", setting, place, sep="_"))
...
{
  Pop   <- readRDS(paste0("data/dem/ADM2_Obs_Alive/",setting,"_", place))
  return(Pop)
  }
...
ADM2_Deaths_import_and_format <- function(place, setting) {
  Deaths <- readRDS(paste0("data/dem/ADM2_Obs_Deaths/", setting, "_", place))
  return(Deaths)
  }
