...
get_parameter_distribution <- function(lambdaNy, place_name, setting_type)
{
  
  Pop      <- Pop_import_and_format(place_name, setting_type)
  Deaths   <- Deaths_import_and_format(place_name)
  ...
  # iters = 100
  param <- read.csv(file='data/pars/Parameters9k.csv') # No Normal
  ...
  #Life Expectancy
  LE   <-  read.csv("data/dem/life expectancy GBD85.csv")
  LE85 <- LE$life.exp
