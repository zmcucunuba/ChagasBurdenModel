...
summ<- matrix(NA,11,6 )
colnames(summ)<- c( "Q2.5_1990" ,"Med_1990", "Q97.5_1990",  "Q2.5_2020", "Med_2020","Q97.5_2020")
...
PopTotalAC[1,]<- c(AC[1],sum(Urban[which(Urban$AÑO=="2020"),7:12]), sum(Rural[which(Rural$AÑO=="2020"),7:12]),
                   sum(Urban[which(Urban$AÑO=="1990"),7:12]), sum(Rural[which(Rural$AÑO=="1990"),7:12]))
PopTotalAC[2,]<- c(AC[2],sum(Urban[which(Urban$AÑO=="2020"),13:27]), sum(Rural[which(Rural$AÑO=="2020"),13:27]),
                   sum(Urban[which(Urban$AÑO=="1990"),13:27]), sum(Rural[which(Rural$AÑO=="1990"),13:27]))
PopTotalAC[3,]<- c(AC[3],sum(Urban[which(Urban$AÑO=="2020"),28:47]), sum(Rural[which(Rural$AÑO=="2020"),28:47]),
                   sum(Urban[which(Urban$AÑO=="1990"),28:47]), sum(Rural[which(Rural$AÑO=="1990"),28:47]))
PopTotalAC[4,]<- c(AC[4],sum(Urban[which(Urban$AÑO=="2020"),48:67]), sum(Rural[which(Rural$AÑO=="2020"),48:67]),
                   sum(Urban[which(Urban$AÑO=="1990"),48:67]), sum(Rural[which(Rural$AÑO=="1990"),48:67]))
PopTotalAC[5,]<- c(AC[5],sum(Urban[which(Urban$AÑO=="2020"),68:107]), sum(Rural[which(Rural$AÑO=="2020"),68:107]),
                   sum(Urban[which(Urban$AÑO=="1990"),68:107]), sum(Rural[which(Rural$AÑO=="1990"),68:107]))

PopTotalAC<- as.data.frame(PopTotalAC)
colnames(PopTotalAC)<- c("AC", "UrbanPop2020", "RuralPop2020", "UrbanPop1990", "RuralPop1990")
PopTotalAC$UrbanPop2020 <- as.numeric(PopTotalAC$UrbanPop2020 )
PopTotalAC$RuralPop2020 <- as.numeric(PopTotalAC$RuralPop2020 )
PopTotalAC$UrbanPop1990 <- as.numeric(PopTotalAC$UrbanPop1990 )
PopTotalAC$RuralPop1990 <- as.numeric(PopTotalAC$RuralPop1990 )

PopTotalAC$TotalPop2020<- PopTotalAC$UrbanPop2020 + PopTotalAC$RuralPop2020
PopTotalAC$TotalPop1990<- PopTotalAC$UrbanPop1990 + PopTotalAC$RuralPop1990
...
summ[1,1:3]<-quantile(TotalCases[,6], c(0.025, 0.5, 0.975))
...
summ[2,1:3]<-quantile(TotalSCases[,6], c(0.025, 0.5, 0.975))
summ[2,4:6]<-quantile(TotalSCases[,36], c(0.025, 0.5, 0.975))

#prevalence totale
summ[3,1:3]<-quantile(TotalCases[,6], c(0.025, 0.5, 0.975))/sum(PopTotalAC$TotalPop1990)
summ[3,4:6]<-quantile(TotalCases[,36], c(0.025, 0.5, 0.975))/sum(PopTotalAC$TotalPop2020)


#prevalence urban
summ[4,1:3]<-quantile(TotalCasesU[,6], c(0.025, 0.5, 0.975))/sum(PopTotalAC$UrbanPop1990)
summ[4,4:6]<-quantile(TotalCasesU[,36], c(0.025, 0.5, 0.975))/sum(PopTotalAC$UrbanPop2020)

#prevalence rural
summ[5,1:3]<-quantile(TotalCasesR[,6], c(0.025, 0.5, 0.975))/sum(PopTotalAC$RuralPop1990)
...
summ[6,1:3]<-quantile(U5[,6], c(0.025, 0.5, 0.975))/PopTotalAC$TotalPop1990[1]
...
summ[7,1:3]<-quantile(A60[,6], c(0.025, 0.5, 0.975))/PopTotalAC$TotalPop1990[5]
...
summ[8,1:3]<-quantile(death[,8], c(0.025, 0.5, 0.975))# deaths from 1992
...
Dep_death1990<- Dep_death[,,8]
DeathMed1990<- t(apply(Dep_death1990, 1,quantile, c(0.025, 0.5, 0.975)))


Dep_death2020<- Dep_death[,,36]
DeathMed2020<- t(apply(Dep_death2020, 1,quantile, c(0.025, 0.5, 0.975)))

write.csv(summ, "ML_inputs/res/summary_table.csv")
...
write.csv(Cases, "ML_inputs/res/Dep_summary_table.csv")
